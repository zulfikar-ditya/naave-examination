        ...
        <table class="table table-responsive">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="">
                            <x-btn-link :link="route('company.company-email.show', ['company' => $company, 'company_email' => $item])" :color="'info'" :value="'Detail'"/>
                            <x-btn-link :link="route('company.company-email.edit', ['company' => $company, 'company_email' => $item])" :color="'warning'" :value="'Edit'"/>
                            <x-btn-link :link="'#'" :color="'danger'" :value="'Delete'" :id="'btn-delete-'.$item->id"/>
                            <form action="{{route('company.company-email.destroy', ['company' => $company, 'company_email' => $item])}}" id="form-delete-{{$item->id}}" method="POST">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
